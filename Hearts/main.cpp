...
void playRound(PlayerClass players[], CardType table[], int& lead, int x, cardSuitType& leadSuit);
void displayHeader();
...
int main() 
{
    ...
    int lead = isFirstLead(players); // find who starts the game (2 of club holder)
...
void dealCards(PlayerClass players[], CardClass gameDeck)
{
    ...
    for (int x = 0; x < MAX_PLAYER_CARDS; x++) 
    {
        for (int i = 0; i < numPlayers; i++) // deal one card to 4 players, one player at a time
        {
...
void playRound(PlayerClass players[], CardType table[], int& lead, int x, cardSuitType& leadSuit)
{
  ...
  for (i = 0; i < numPlayers; i++)
  {
    if (x == 0)//frst round of game
    {
      ...
      else if ((lead + i) % 4 == 0 && i != 0)//first round for user but not first play
      {
    ...
    else // if round is not 0
    {
      ...
      else if ((lead + i) % 4 == 0 && i != 0)// user and not first play of round
      {
        table[i] = promptUser2(players, leadSuit);
      }
      else//computers
...
void showUser(PlayerClass players[])
{
    ...
    cout << "#" << setw(10) << "SUIT" << setw(12) << "VALUE" << setw(12) << "POINTS" << endl;
...
void addScores(CardType table[], PlayerClass players[], int lead)
{
  ...
  switch (lead)
    ...
    case 0:
      ...
      break;
    case 1:
      cout << "-----Computer " << lead << " gets "<< sum << " points-----" << endl;
      players[lead].AddScore(sum);
      playerScore = players[lead].GetScore();
      break;
    case 2:
...
void revealWinner(PlayerClass players[])
{
  ...
  if (index == 0)
  {
    cout << "You are the winner with " << players[index].GetScore() << " points!" << endl;
  }
  else 
  {
    cout << "Computer " << index << " is the winner with " << players[index].GetScore() << " points!" << endl;
  }
}