...
CardType PlayerClass::FollowOneCard(cardSuitType suit) {
  ...
  if (cardCount >= 1 && cardCount <= MAX_PLAYER_CARDS) {
    ...
    // If no matching suit found, play the first card
    temp = hand[0];
  ...
  // Return a default card if no valid cards in hand
  return CardType();
}
...
bool PlayerClass::IsFirstLead() const {
  ...
  if (cardCount == 13) // enter if player's hand is full
  {
    int index = 0;
    while (!found &&
           index < cardCount) // keep searching until 2 of clubs is found
    {
...
void PlayerClass::Sort() {
  if (cardCount >= 1) {
    ...
    for (int i = 0; i < cardCount - 1; i++) {
      ...
      for (int j = i + 1; j < cardCount;
           j++) // start from next card in player's hand and compare
      {
      ...
      swap(hand[index], hand[i]); // swap the two cards, putting the lowest
                                  // found card at the top most position
    }
